...
class AEGraph:
    ...
    cost: list[list[float]] = field(default_factory=list)
    ...
    def extract(self, eid):
        self.cost = [0.0] * len(self.uf)
        for eid in range(len(self.uf)):
            for eid1 in self.enum(eid):
                enode = self.enodes[eid1]
                sum(self.cost[arg] for arg in enode.args)
            self.cost[eid] = size(self.enodes[eid])
